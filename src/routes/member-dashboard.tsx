...
export const Route = createFileRoute("/member-dashboard")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Your LOUDMOUF™ member portal — membership status, personal allocation tracker, yield history, rewards balance and community updates." },
      { property: "og:title", content: "Member Dashboard — LOUDMOUF™" },
      { property: "og:description", content: "Your LOUDMOUF™ personal allocation tracker, yield history and rewards balance." },
    ],
  }),
...
function MemberDashboard() {
  ...
  const tierLabel = member?.tier === "premium" ? "Premium" : member?.tier === "standard" ? "Standard" : "Guest";
    ...
    <div className="relative min-h-screen overflow-x-clip bg-background text-foreground">
      ...
      <section className="relative pt-36 pb-10 sm:pt-40">
        ...
        <div className="relative mx-auto max-w-6xl px-6">
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-xs uppercase tracking-[0.3em] text-loud-yellow">
            Member Portal
          </motion.p>
      ...
      <section className="relative mx-auto max-w-6xl px-6 pb-24 space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card icon={CreditCard} title="Membership" body={`${tierLabel} · R${member?.tier === "premium" ? 149 : 99}/mo`} cta={{ label: "Manage plan", href: "/membership" }} />
          <Card icon={Leaf} title="Current Cycle" body="0 yield shares requested" cta={{ label: "Request allocation", href: "/#product" }} />
          <Card icon={Wallet} title="Rewards Balance" body={member?.tier === "premium" ? "0 pts · Cashback active" : "Upgrade to Premium to earn"} cta={{ label: member?.tier === "premium" ? "View rewards" : "Upgrade", href: "/membership" }} />
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <StatBlock icon={Calendar} title="Events" hint="Launch Summit · 11 Aug 2026" href="/launch" />
        ...
        {!member && (
          <div className="rounded-3xl border border-loud-yellow/30 bg-loud-yellow/5 p-6 text-sm text-white/80">
            <p className="font-semibold uppercase tracking-widest text-loud-yellow text-xs">Not a member yet?</p>
...
function Card({ icon: Icon, title, body, cta }: { icon: typeof Sparkles; title: string; body: string; cta: { label: string; href: string } }) {
  return (
    <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6 backdrop-blur-md">
      ...
      <Link to={cta.href} className="mt-4 inline-flex text-xs uppercase tracking-widest text-white/70 hover:text-loud-yellow">
        {cta.label} →
      </Link>
    </div>
  );
...
function StatBlock({ icon: Icon, title, hint, href }: { icon: typeof Sparkles; title: string; hint: string; href?: string }) {
  const Wrap: React.ElementType = href ? Link : "div";
  const props = href ? { to: href } : {};
  return (
    <Wrap {...props} className="block rounded-2xl border border-white/10 bg-white/[0.02] p-5 hover:border-loud-yellow/30 transition">
